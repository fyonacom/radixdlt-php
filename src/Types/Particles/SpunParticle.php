    ...
    implements ToJsonInterface, FromJsonInterface
{
    ...
    protected function __construct(protected AbstractParticle $particle, protected int $spin)
    {
    }

    public function up(AbstractParticle $particle): SpunParticle
    {
        return new self($particle, self::SPIN_UP);
    }

    public function down(AbstractParticle $particle): SpunParticle
    {
        return new self($particle, self::SPIN_DOWN);
    }

    public static function fromJson(array|string $json): SpunParticle
    {
        if (is_string($json)) {
